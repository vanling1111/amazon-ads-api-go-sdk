/*
 * Amazon Advertiser API for Retail Ad Service
 *
 * The API that enables advertisers to run campaigns through Amazon Ads on retailers that are launched on Amazon Retail Ad Service
 *
 * API version: 3.0
 * Generated by: Swagger Codegen (https://github.com/swagger-api/swagger-codegen.git)
 */
package retailadservice

type Rasv1ParentEntityErrorReason string

// List of RASv1ParentEntityErrorReason
const (
	PARENT_ENTITY_NOT_FOUND_Rasv1ParentEntityErrorReason Rasv1ParentEntityErrorReason = "PARENT_ENTITY_NOT_FOUND"
)
